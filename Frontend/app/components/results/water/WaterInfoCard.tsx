...
const tooltips: Record<string, string> = {
  ...
  "Συγκέντρωση ιόντων υδρογόνου":
    "Το pH δείχνει αν το νερό είναι όξινο ή αλκαλικό.",
  "Υπολειμματικό χλώριο":
    "Δείχνει την ποσότητα χλωρίου που παραμένει μετά την απολύμανση.",
}

const parameterLimits: Record<string, string> = {
  "Θολότητα NTU": "< 1.0 NTU",
  Χρώμα: "<= 15 mg/l Pt-Co",
  Αργίλιο: "<= 200 μg Al/l",
  Χλωριούχα: "<= 250 mg/l",
  Αγωγιμότητα: "<= 2500 μS/cm",
  "Συγκέντρωση ιόντων υδρογόνου": "6.5 - 9.5 pH",
  "Υπολειμματικό χλώριο": ">= 0.2 mg/l",
}
...
function getLimitLabel(param: string): string | null {
  return parameterLimits[param] ?? null
}
      ...
      <div className="rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-50 to-white px-5 py-4 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-xl">
            <h4 className="text-base font-semibold tracking-tight text-[#1a535c] sm:text-lg">
              Λεπτομέρειες μήνα
            </h4>
          ...
          {activeYear && (
            <div className="flex shrink-0 items-center self-start sm:self-auto">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3.5 py-1.5 text-xs font-semibold text-sky-800 shadow-sm ring-1 ring-sky-200 backdrop-blur">
      ...
      {!measurements.length ? (
        ...
        <ul className="mt-4 space-y-3">
          {measurements.map((item, idx) => {
            ...
            const pct =
              typeof item.pct === "number" ? `${item.pct.toFixed(1)}%` : null
            const displayValue = hasValue ? String(item.val) : "Χωρίς δεδομένα"
            const limitLabel = getLimitLabel(item.param)
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1 text-sm leading-6 text-[#1a535c]">
                  <span className="font-semibold">{item.param}</span>:{" "}
                  {displayValue}
                  {pct ? (
                    <span className="ml-2 text-[#1a535c]/65">
                      (Ποσοστό ορίου: {pct})
                    </span>
                  ) : null}
                  {limitLabel ? (
                    <div className="mt-1 text-xs font-medium text-[#1a535c]/58">
                      Όριο αναφοράς: {limitLabel}
                    </div>
                  ) : null}
                </div>
